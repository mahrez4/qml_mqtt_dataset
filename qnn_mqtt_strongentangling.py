...
# Quantum Node
# ======================================================
def create_qnode(n_qubits=NUM_QUBITS, depth=2):
    dev = qml.device("default.qubit", wires=n_qubits)

    @qml.qnode(dev, interface="torch", diff_method="backprop")
    def qnode(inputs, weights):
        # Consider scaling inputs beforehand (e.g., to [-pi, pi])
        qml.templates.AngleEmbedding(inputs, wires=range(n_qubits), rotation="Y")
        qml.templates.StronglyEntanglingLayers(weights, wires=range(n_qubits))
        return [qml.expval(qml.PauliZ(i)) for i in range(n_qubits)]

    # TorchLayer will feed a "weights" tensor shaped (depth, n_qubits, 3)
    return qnode
...
class QNNClassifier(nn.Module):
    def __init__(self, n_qubits=NUM_QUBITS, num_classes=4, depth=2):
        super().__init__()
        self.n_qubits = n_qubits
        self.depth = depth
        self.qnode = create_qnode(n_qubits, depth)
        weight_shapes = {"weights": (depth, n_qubits, 3)}   # <-- IMPORTANT
        self.q_layer = qml.qnn.TorchLayer(self.qnode, weight_shapes)
        self.classifier = nn.Linear(n_qubits, num_classes)

    def forward(self, x, return_features=False):
        # x shape: (B, num_features)
        x = x[:, :self.n_qubits]  # trim if > n_qubits (ensure >= n_qubits elsewhere)
        q_out = self.q_layer(x)   # shape: (B, n_qubits), values in [-1, 1]
...
def train_joint(model, dataloader, optimizer, criterion_cls, criterion_cont, device):
    ...
    for features, labels in tqdm(dataloader, desc="Training"):
        ...
        loss_cont = criterion_cont(z_clean, z_noisy)   # <-- embeddings, not logits
